...
% Last Modified by GUIDE v2.5 26-Jun-2021 08:05:40
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
opts = detectImportOptions('DATARUMAH.xlsx'); %IMPORT DATA
opts.SelectedVariableNames = (1);
data1 = readmatrix('DATARUMAH.xlsx',opts);
%data di import dari excel
...
%proses perhitungan perankingan
for i=1:m
 V(i)= sum(w.*R(i,:));
end
%menampilkan hasil perhitungan rekomdenasi
Vtranspose=V.'; 
Vtranspose=num2cell(Vtranspose);
opts = detectImportOptions('DATARUMAH.xlsx');
opts.SelectedVariableNames = (2);
hasil= readtable('DATARUMAH.xlsx',opts);
hasil = table2cell(hasil);
hasil=[hasil Vtranspose];
hasil=sortrows(hasil,-2);
hasil = hasil(1:20,1:2);
%data hasil rekomendasi perhitungan dimasukan ke tabel 2
set(handles.uitable2, 'data', hasil, 'visible','on');
